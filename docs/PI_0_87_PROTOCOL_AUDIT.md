...
## Current status — local fixes, not deployed

After the user requested continuation, fixed the reproduced adapter/extension problems **in this isolated branch only**. The running signed Kraki binary remains 0.31.22 and has NOT received these fixes.
...
- The isolated branch contains validated fixes, but **no production fixes have been deployed**. The two answer-loss bugs still affect the running unpatched release until a new signed build is released/installed.
