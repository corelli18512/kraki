...
const mockDevices = [
  { name: 'CI Server', role: 'tentacle', kind: 'server', agent: { type: 'code', id: 'copilot', models: ['claude-sonnet-4', 'gpt-4.1'] } },
  { name: 'Linux Dev VM', role: 'tentacle', kind: 'vm', agent: { type: 'code', id: 'claude', models: ['claude-sonnet-4', 'gpt-4.1', 'o3'] } },
  { name: 'Work Laptop', role: 'tentacle', kind: 'desktop', agent: { type: 'code', id: 'copilot', models: ['gpt-4o'] } },
];
...
function sendGreeting(ws: WebSocket, targetDeviceId: string, dev: typeof mockDevices[0], myDeviceId: string) {
  ws.send(JSON.stringify({
    ...
    payload: { name: dev.name, kind: dev.kind, agent: dev.agent },
  }));
}
