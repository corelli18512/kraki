 ...
 *   • `schedule_wake(...)` — asks the Tentacle-owned durable scheduler to wake
 *     this same session at a future time. The session scope comes from the
 *     loopback MCP URL, not from model-supplied arguments.
...
// Registers ask_user + finalize_reply + show_image + schedule_wake +
// kraki_get_mode and a per-call permission gate that
...
const KRAKI_TOOLS = new Set(["ask_user", "finalize_reply", "show_image", "show_html", "kraki_get_mode", "schedule_wake"]);
...
export default function krakiTools(pi) {
  ...
  pi.registerTool({
    name: "schedule_wake",
    label: "Schedule agent wake",
    description:
      "Schedule Kraki to wake this same agent session in the future. Use 'at' for " +
      "a one-time ISO-8601 timestamp, or 'cron' plus 'timezone' for a recurring wake. " +
      "The Agent process may exit; Tentacle persists and later restores the session.",
    promptGuidelines:
      "Use schedule_wake when the user asks this agent to do something later or repeatedly. " +
      "Tell the user the exact schedule returned by the tool.",
    parameters: {
      type: "object",
      properties: {
        instruction: { type: "string", description: "Task to execute when this session is woken." },
        label: { type: "string", description: "Optional short user-facing name." },
        at: { type: "string", description: "One-time ISO-8601 timestamp with Z or an explicit UTC offset." },
    ...
    async execute(_id, params) {
      const url = process.env.KRAKI_MCP_URL;
      const token = process.env.KRAKI_MCP_TOKEN;
      if (!url || !token) {
        return { content: [{ type: "text", text: "schedule_wake: Kraki scheduler is unavailable" }], isError: true, details: {} };
      }
      try {
        const response = await fetch(url, {
          ...
          body: JSON.stringify({
            ...
            id: "schedule-wake",
            method: "tools/call",
            params: { name: "schedule_wake", arguments: params || {} },
          }),
        });
        const rpc = await response.json();
        if (!response.ok || rpc.error) {
          const message = (rpc.error && rpc.error.message) || ("HTTP " + response.status);
          throw new Error(message);
        }
        return { ...(rpc.result || {}), details: { scheduled: !rpc.result?.isError } };
      } catch (err) {
        const message = (err && err.message) || String(err);
        return { content: [{ type: "text", text: "schedule_wake: " + message }], isError: true, details: {} };
      }
    },
  });
