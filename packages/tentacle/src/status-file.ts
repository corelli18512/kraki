...
export interface DaemonStatusFile {
  ...
  relay: string;
  deviceName: string;
  activeSession: { id: string; title?: string; state: string } | null;
  updatedAt: number;
}
...
export function updateActiveSession(session: DaemonStatusFile['activeSession']): void {
  _current = { ..._current, activeSession: session, updatedAt: Date.now() };
  writeStatus();
}
