...
import { MAX_DIMENSION as SHOW_IMAGE_MAX_DIMENSION } from '../../image-resize.js';
import { vi } from 'vitest';

vi.mock('../../image-resize.js', () => ({
  fitToMaxDimension: vi.fn((bytes: Buffer, mime: string) =>
    Promise.resolve({ bytes, mimeType: mime })),
  MAX_DIMENSION: 2000,
}));
...
describe('show_image handler — downscale', () => {
  ...
  it('calls fitToMaxDimension and uses resized result', async () => {
    const { fitToMaxDimension: mockFit } = await import('../../image-resize.js');
    const over = Buffer.alloc(1);
    (mockFit as ReturnType<typeof vi.fn>).mockResolvedValueOnce({ bytes: over, mimeType: 'image/jpeg' });

    const p = join(dir, 'pic.png');
    writeFileSync(p, PNG_1X1_BYTES);
    const r = await showImageHandler({ path: p }, ctx);
    expect(r.isError).toBeFalsy();
    expect((r.content[0] as { data: string }).data).toBe(over.toString('base64'));
    expect((r.content[0] as { mimeType: string }).mimeType).toBe('image/jpeg');
  });

  it('falls back to original bytes when resize throws', async () => {
    const { fitToMaxDimension: mockFit } = await import('../../image-resize.js');
    (mockFit as ReturnType<typeof vi.fn>).mockRejectedValueOnce(new Error('boom'));

    const p = join(dir, 'pic.png');
    writeFileSync(p, PNG_1X1_BYTES);
    const r = await showImageHandler({ path: p }, ctx);
    expect(r.isError).toBeFalsy();
    expect((r.content[0] as { data: string }).data).toBe(PNG_1X1);
  });
});
