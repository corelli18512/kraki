...
fn build_menu(
    ...
    let session_label = session_label(status);
    ...
    let session_item = MenuItem::with_id(app, "session", session_label, false, None::<&str>)?;
...
fn session_label(status: &DaemonStatus) -> String {
    match &status.active_session {
        Some(s) => {
            let id_short = &s.id[..s.id.len().min(8)];
            let label = s.title.as_deref().unwrap_or(id_short);
