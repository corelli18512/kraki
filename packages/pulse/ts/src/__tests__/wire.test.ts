...
describe('wire codec — 64-bit and UTF-8 edge cases', () => {
  ...
  it('measures str length in UTF-8 bytes, not code points', () => {
    const f: Frame = { t: 'hello', epoch: '🐙', recvEpoch: '', recvCursor: 0n, durableSupported: false, maxRetentionMs: 0n };
  ...
  it('rejects an epoch longer than 255 UTF-8 bytes at encode time', () => {
    const f: Frame = { t: 'hello', epoch: 'x'.repeat(256), recvEpoch: '', recvCursor: 0n, durableSupported: false, maxRetentionMs: 0n };
