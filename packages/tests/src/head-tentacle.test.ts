...
describe("Thin Relay Integration: Head + Tentacle + App", () => {
  ...
  it("3. app sends approve via unicast → adapter receives", async () => {
    ...
    expect((ca.payload as { action: { type: string } }).action.type).toBe("permission");
    expect((ca.payload as { action: { payload: { id: string } } }).action.payload.id).toBe("perm_1");
  ...
  it("5. permission deny flows from app to adapter", async () => {
    ...
    expect((perm.payload as { action: { payload: { id: string } } }).action.payload.id).toBe("perm_2");
  ...
  // ── 6. Streaming (agent_message_delta draft) ────────────────

  it("6. agent_message_delta draft broadcasts arrive at app (coalesced within debounce window)", async () => {
    const app = await connectApp(env.port);
    await connectTentacle();
  ...
  it("broadcast with pushPreview is forwarded to online app via WS (push requires PushManager)", async () => {
    ...
    expect((perm.payload as { action: { payload: { id: string } } }).action.payload.id).toBe("perm-1");
